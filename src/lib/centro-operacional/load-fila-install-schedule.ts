...
import type { FilaInstallScheduleItem } from "./fila-install-schedule";
import { resolveFilaClienteNome } from "./fila-cliente-nome";

export async function loadFilaInstallSchedule(unidadeId?: string | null): Promise<{
  ...
  let osQuery = supabase
    .from("ordens_servico")
    .select(
      "id, cliente_id, titulo, atualizado_em, equipe_id, equipe_atual_id, status_atual, etapa_atual, data_prevista_material",
    )
    .eq("ativo", true)
    .eq("etapa_atual", "install_schedule");
  ...
  const fila: FilaInstallScheduleItem[] = [];
  ...
  for (const os of osRows) {
    ...
    fila.push({
      ...
      statusAtual: (os.status_atual as string) ?? "install_schedule_pending",
      instalacaoAgendada: inst?.agendada ?? false,
      instalacaoQuando: inst?.quando ?? null,
      dataPrevistaMaterial: os.data_prevista_material
        ? String(os.data_prevista_material).slice(0, 10)
        : null,
